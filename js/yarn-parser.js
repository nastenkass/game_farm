function captureSquareBrackets(line) {
  // Находим строку "[[...]]" и захватываем ее содержимое
  var regExp = /\[\[(.*?)\]\]/i;
  ...
  if (capture == null) {
    ...
    // Текст и идентификатор следующего узла находятся внутри [[здесь]]
    // Разделите их на | характер, но ограничив разделение до первой | после "]]"
    var data = capture[1].split('|', 2);
    var choice = { "target": "", "text": "", "actions": [] };
    ...
    // Поиск и обработка ((...))
    var actionsRegExp = /(\<\<(.*?) (.*?)\>\>)+/g;
    var actionsCapture = actionsRegExp.exec(choice.text);
    while (actionsCapture !== null) {
      // Добавляем найденные действия в массив
      choice.actions.push(actionsCapture[1]);
      // Продолжаем поиск следующего вхождения
      actionsCapture = actionsRegExp.exec(choice.text);
    }
    return choice;
  }
}
...
function parseStory()
{
  ...
  for(var node in story)
  {
    ...
    for(var l in lines)
    {
      ...
      if (line.substring(0,2) == "//") continue; //Ignore comments
      ...
      {
        //Удалите символьные данные из строки, чтобы продолжить ее правильный анализ.
